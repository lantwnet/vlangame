...
/// 客户端运行句柄，用于优雅停机。
/// 调用 [`ClientHandle::shutdown`] 后：
/// - WinDivert 模式：调用 `WinDivert::shutdown` 解除阻塞的 recv 线程，并释放虚拟 IP
/// - TUN 模式：各 channel 随任务自然关闭
...
impl ClientHandle {
    pub fn shutdown(self) {
        ...
        if let Some(divert) = &self.windivert {
            ...
            // 包括与 WinDivertRecv 并发。Rust wrapper 的 &mut self 限制过于保守。
            unsafe {
                let ptr = Arc::as_ptr(divert) as *mut WinDivert<NetworkLayer>;
                if let Err(e) = (*ptr)
                    .shutdown(windivert::prelude::WinDivertShutdownMode::Both)
                {
                    log::warn!("WinDivert shutdown: {e:?}");
                }
            }
        }
        // ip_release 在此 drop，自动移除虚拟 IP
    }
}
